...
from rules.CommitRevealDetector import CommitRevealDetector
...
class TestCommitRevealDetector(unittest.TestCase):
    def test_detects_vulnerability(self):
        violations = run_rule_on_file("test_contracts/CommitRevealDetector_bad.sol", CommitRevealDetector)
        self.assertTrue(len(violations) > 0, "Vulnerability should be detected")

    def test_ignores_safe_contract(self):
        violations = run_rule_on_file("test_contracts/CommitRevealDetector_good.sol", CommitRevealDetector)
